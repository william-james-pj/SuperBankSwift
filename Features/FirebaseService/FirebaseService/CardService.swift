...
public class CardService: CardNetwork {
    ...
    public func saveInvoice(_ invoice: InvoiceModel) async throws {
        do {
            let newObjRef = dataBase.collection("invoices").document()
    ...
    public func savePhysicalCard(accountId: String) async throws {
        do {
            ...
            let newObjRef = dataBase.collection("cards").document()
    ...
    public func getAllCard(accountId: String) async throws -> [CardModel] {
        let query = dataBase.collection("cards").whereField("accountId", isEqualTo: accountId)
        ...
        for document in documents.documents {
            ...
            let isInternationalPurchasesEnabled = data["isInternationalPurchasesEnabled"] as? Bool ?? false
    ...
    public func updateBlockers(cardId: String, keyName: String, value: Bool) async throws {
        do {
            let newObjRef = dataBase.collection("cards").document(cardId)
    ...
    private func getCustomerName(accountId: String) async throws -> String {
        do {
            ...
            let qCustomer = dataBase.collection("customers").document(customerId)
    ...
    private func getCustomerId(accountId: String) async throws -> String {
        let qAccount = dataBase.collection("accounts").document(accountId)
